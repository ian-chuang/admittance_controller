...
controller_interface::return_type AdmittanceRule::reset(const size_t num_joints)
{
  ...
  // reset wrench zeroing variables
  zero_wrench_flag_ = true;
  zero_wrench_value_.setZero();
  ...
  const Eigen::Matrix<double, 3, 3> & cog_world_rot)
{
  ...
  if (zero_wrench_flag_)
  {
    for (size_t i = 0; i < 6; ++i)
    {
      zero_wrench_value_(i) = new_wrench_base(i);
    }
    zero_wrench_flag_ = false;
  }

  // zero out the wrench
  for (size_t i = 0; i < 6; ++i)
  {
    new_wrench_base(i) -= zero_wrench_value_(i);
  }
